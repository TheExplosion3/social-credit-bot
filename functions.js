...
function observer() {
  ...
  // this could be improved with a better regex, but i don't really understand it to its full potential yet so I cant do so.
  ...
  collector.on('end', collected => {
    ...
    if(collected !== null) {
      for(let m in collected) {
        ...
        msgid_creditchange.set(arr[idx], (m.matchAll(posregex).length - 1) * words.wordscore)
        while(true) {
          ...
          currentarr = m.matchAll(posregex);
          modstring = m.slice(idxtwo, m.matchAll(posregex).length - 1);
          if(modstring.match(negregex) !== null) {
            // sets wordscore based upon negative words, reducing if theyre found
            msgid_credit.set(arr[idx], (msgid_credit.get(idx) - (modstring.match(negregex).length) - 1) * words.wordscore)
    ...
    for(const [key, value] of msgid_creditchange.entries()) {
      sc_change(true, value, key);
    }
  });        
}